...
(() => {
  ...
  function ensureStage(){
    ...
    stage.innerHTML=`
      <div class="postcard-preview-head">
        ...
        <button type="button" class="postcard-close" aria-label="Close postcard preview">×</button>
      </div>
      <div class="postcard-scene" aria-label="3D postcard preview">
        <div class="postcard-card" tabindex="0" role="button" aria-label="Flip postcard">
    ...
    stage.addEventListener('click',e=>{if(e.target===stage)closePreview()});
    $('.postcard-close',stage).addEventListener('click',closePreview);
    $('.postcard-flip-btn',stage).addEventListener('click',flip);
    $('.postcard-card',stage).addEventListener('click',e=>{if(e.target.closest('button'))return;flip()});
  ...
  function openPreview(){
    ...
    const img=$('#postcardFrontImg');
    img.src=currentProduct.image;
    img.alt=currentProduct.title+' postcard preview';
    $('#postcardFrontTitle').textContent=currentProduct.title;
    $('.postcard-card',stage).classList.remove('is-back');
    stage.classList.add('open');
    stage.setAttribute('aria-hidden','false');
    open=true;
    document.body.classList.add('postcard-preview-open');
  }
  function closePreview(){
    const stage=$('#postcardPreview'); if(!stage)return;
    stage.classList.remove('open'); stage.setAttribute('aria-hidden','true'); open=false;
    document.body.classList.remove('postcard-preview-open');
  }
  ...
  window.UMS91PostcardPreview={setProduct(p){currentProduct=p;bind()},open:openPreview,close:closePreview};

  const originalOpenProduct=window.openProduct;
  // script.js exposes openProduct only inside its module/closure, so instead
  // observe the modal title/image after it opens and capture the matching product.
  ...
  function init(){
    ensureStage();
    const modal=$('#productModal');
    if(modal)modalObserver.observe(modal,{subtree:true,childList:true,attributes:true,attributeFilter:['src','class']});
    bind();
